...
`;function ci(e){if(typeof CSSStyleSheet<"u"&&"adoptedStyleSheets"in Document.prototype)try{const n=new CSSStyleSheet;n.replaceSync(Ut),e.adoptedStyleSheets=[...e.adoptedStyleSheets,n];return}catch{}const t=document.createElement("style");t.textContent=Ut,e.appendChild(t)}function qt(){const e=[document.body,document.documentElement].filter(Boolean);for(const t of e){const n=getComputedStyle(t).backgroundColor,r=ui(n);if(r!==null)return r<.4?"dark":"light"}return typeof matchMedia=="function"&&matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"}function ui(e){const t=e.match(/rgba?\(([^)]+)\)/);if(!t)return null;const n=t[1].split(",").map(s=>Number.parseFloat(s.trim()));if(n.length<3||n.some(Number.isNaN)||n.length>3&&n[3]===0)return null;const[r,i,o]=n;return(.2126*r+.7152*i+.0722*o)/255}const Gt="patchlet-widget",pi="patchlet_ask";function di(){return new URLSearchParams(location.search).get(pi)?.trim()??""}function fi(){const t=document.currentScript??document.querySelector("script[data-key]"),n=t?.dataset.key?.trim();if(!n)return console.warn("[patchlet] no data-key on the script tag, the widget will not load"),null;const r=t?.src?new URL(t.src,location.href).origin:location.origin,i=t?.dataset.api?.trim()||r,o=t?.dataset.position==="left"?"left":"right";return{key:n,apiBase:i,position:o}}function jt(e){if(document.querySelector(Gt))return;let t=di();const n=document.createElement(Gt);n.setAttribute("data-pl-scheme",qt()),document.body.appendChild(n);const r=n.attachShadow({mode:"open"});ci(r);const i=document.createElement("div");r.appendChild(i);const o=new MutationObserver(()=>n.setAttribute("data-pl-scheme",qt()));o.observe(document.documentElement,{attributes:!0,attributeFilter:["class","style","data-theme"]}),o.observe(document.body,{attributes:!0,attributeFilter:["class","style","data-theme"]});const s=new li({apiBase:e.apiBase,key:e.key});en(d(Zr,{client:s,shadow:r,host:n,position:e.position,register:l=>{if(window.Patchlet=l,t){const p=t;t="",setTimeout(()=>window.Patchlet?.ask(p),400)}}}),i)}function hi(){const e=fi();e&&(document.body?jt(e):document.addEventListener("DOMContentLoaded",()=>jt(e),{once:!0}))}hi()})();
